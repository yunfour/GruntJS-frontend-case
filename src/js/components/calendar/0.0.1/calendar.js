...
define(function(require) {
    ...
    // 日历模板
    var TEMPLATE = [
        '<div class="sea-calendar" style="display:none;">',
            ...
            '</h6>',
            '<div class="sea-calendar-date">',
                '<ul class="sea-calendar-week"></ul>',
                '<ul class="sea-calendar-day clearfix"></ul>',
            '</div>',
            '<ul class="sea-calendar-years"></ul>',
            '<ul class="sea-calendar-monthes"></ul>',
        '</div>'
    ].join('');
        ...
        methods: {
            ...
            renderUI: function() {
                var that         = this,
                    ...
                    widgetEle    = $(that.getAttr('template')),                     // 组件根节点
                    ...
                    monthLstEle  = widgetEle.find('ul.sea-calendar-monthes');       // “月”列表节点
            ...
            bindUI: function() {
                var that        = this,
                    ...
                    calendarBd  = that.getAttr('calendarBd'),
                    yearLstEle  = that.getAttr('yearLstEle'),
                    monthLstEle = that.getAttr('monthLstEle'),
                ...
                widgetEle.on('click', 'ul.sea-calendar-years a', function() {
                    var yearItm      = $(this),
                        year         = yearItm.data('year'),
                        selectedDate = parseDate(that.getAttr('selectedDate') || new Date()),
                    ...
                    if(year === 'next' || year === 'pre') {
                        ...
                        yearLstEle.find('a')
                                  .removeClass('active')
                                  .filter('a[data-year=' + selectedDate.getFullYear() + ']')
                                  .addClass('active');
                ...
                widgetEle.on('click', 'ul.sea-calendar-day a', function() {
                    var dayItm       = $(this),
                        hideOnSelect = that.getAttr('hideOnSelect'),
                        newDate      = parseDate(dayItm.data('date'));
                ...
                widgetEle.on('click', 'h6.sea-calendar-title a.J-operate', function() {
                    var operateBtn = $(this),
                        viewDate   = parseDate(that.getAttr('viewDate') || new Date()),
                        month      = 0,
                        newDate,
                        eventName;
            ...
            setTheme: function(themeName) {
                ...
                widgetEle.removeClass()
                         .addClass('sea-calendar sea-calendar-theme-' + themeName);
    ...
    function renderWeek() {
        var that           = this,
            weekLstEle     = that.getAttr('weekLstEle'),
            weekNamesShort = that.getAttr('weekNamesShort'),
            weekNames      = that.getAttr('weekNames'),
            weekHtml       = [];
    ...
    function renderYearLst(beginYear) {
        var that         = this,
            yearLstEle   = that.getAttr('yearLstEle'),
            viewDate     = that.getAttr('viewDate') || new Date(),
            yearLstHtml  = [];
    ...
    function renderMonthLst() {
        var that         = this,
            monthLstEle  = that.getAttr('monthLstEle'),
            monthNames   = that.getAttr('monthNames'),
            monthLstHtml = [];
    ...
    function renderCalendar(theDate) {
        var that        = this,
            widgetEle   = that.getAttr('widgetEle'),
            dayLstEle   = widgetEle.find('ul.sea-calendar-day'),
            yearEle     = that.getAttr('yearEle'),
            yearLstEle  = that.getAttr('yearLstEle'),
            monthEle    = that.getAttr('monthEle'),
            monthLstEle = that.getAttr('monthLstEle'),
            theRange    = that.getAttr('range'),
            ...
            // theDate对应的年，如theDate=2012年12月23日，则year指的2012
            year,
            
            // theDate对应的月，如theDate=2012年12月23日，则month指的12
            month,
            
            // theDate对应的日，如theDate=2012年12月23日，则day指的23
            day,
            
            // theDate对应的月一共有多少天
            days,
            
            // theDate所在月的1号的日期，如theDate=2012年12月23日，则monthFirstDay指的是2012年12月1日
            monthFirstDay,
            
            /* monthFirstDayWeek指的是theDate对应月的1号是星期几，如theDate=2012
        ...
        allYearItms.removeClass('active')
                   .filter('a[data-year=' + year + ']')
                   .addClass('active');
        yearEle.html(year + '年');
