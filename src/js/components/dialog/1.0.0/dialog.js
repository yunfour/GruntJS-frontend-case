...
define(function(require, exports, module) {
    // 添加样式
    seajs.importStyle([
        '.sea-dialog{ padding: 7px; background: #999; position: absolute; top: 0; left: 0; z-index: 10000;}',
        '.sea-dialog-panel{ background: #fff; position: relative;}',
        '.sea-dialog-title{ height: 30px; padding: 0 5px; margin: 0; line-height: 30px; border-bottom: 1px solid #eee; font-size: 13px; font-weight: bold; color: #333; position: relative;}',
        '.sea-dialog-bd{ padding: 10px;}',
        '.sea-dialog-close{ display: block; height: 14px; width: 14px; font-size: 14px; font-weight: bold; line-height: 14px; color: #333; text-align: center; font-family: Dotum; text-decoration: none; overflow: hidden; position: absolute; top: 6px; right: 6px;}',
        '.sea-dialog-close:hover{ color: #333; text-decoration: none;}'
    ].join(''));

    var $ = require('$');
    
    var dialogHtml = [
        '<div class="sea-dialog">',
            '<div class="sea-dialog-panel">',
                '<h4 class="sea-dialog-title"></h4>',
                '<div class="sea-dialog-bd"></div>',
                '<a class="sea-dialog-close" href="javascript:;" title="点击关闭窗口">x</a>',
            '</div>',
        '</div>'
    ].join('');
    
    var createClass = require('base/createClass/1.0.0/createClass'),
        singleton   = require('base/singleton/1.0.0/singleton'),
    ...
    var Dialog = createClass({
        ...
        init: function(conf) {
            var that           = this,
                dialogEle      = $(dialogHtml),
                dialogCloseEle = dialogEle.find('a.sea-dialog-close');
        ...
        methods: {
            ...
            setWidth: function(width) {
                ...
                that.getAttr('dialogEle').css({width: width});
            ...
            setPosition: function(x, y) {
                ...
                top  = $win.height() * 618 / (1000 + 618) - dialogEle.height() / 2;
                ...
                dialogEle.css({left: x, top: y, position: conf.position});
    ...
    Dialog.confirm = function(tipTxt, callback) {
        var contentEle = $([
            '<p style="padding:15px 0 0;margin:0;font-size:14px;color:#666;line-height:1.5;text-align:center;">' + tipTxt + '</p>',
            '<div style="padding:15px 0;text-align:center;font-size:13px;">',
                '<a class="J-confirmBtn" href="javascript:;" style="color:#333;">确定</a>&nbsp;&nbsp;&nbsp;&nbsp;',
                '<a class="J-cancelBtn" href="javascript:;" style="color:#999;">取消</a>',
            '</div>'
        ].join(''));
        
        var dialog = Dialog.confirm._getConfirmDialogObj();
        
        var closeBtn    = dialog.getAttr('dialogCloseEle'),
            confirmBtn  = contentEle.find('a.J-confirmBtn'),
            cancelBtn   = contentEle.find('a.J-cancelBtn');
    ...
    Dialog.alter = function(tipTxt) {
        ...
        var confirmBtn = contentEle.find('a.J-confirmBtn'),
            closeBtn = dialog.getAttr('dialogCloseEle');
