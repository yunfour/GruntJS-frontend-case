define(function(require) {
    ...
    function setFilter(filterData, filterPanel, pagerPanel) {
        var fields    = filterPanel.find('input,textarea,select'),
            filterBtn = filterPanel.find('a.J-filterBtn'),
            fieldSet  = {},
            filterUrl = filterPanel.attr('data-filter-url');
        ...
        if(pagerPanel) {
            pagerPanel.delegate('li', 'click', function() {
                var btn = $(this),
                    page = filterData.currentPage || pagerPanel.find('li.active').data('page') || 1;
